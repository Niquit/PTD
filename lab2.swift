...
for i in 1...(Times.count) {
    ...
    for n in 0..<(Times.count) {
        real = Float(real!) + Tones[n] * cos(2*Float.pi * Float(n) * Float(i) / Float(Times.count))
        imag = Float(imag!) - Tones[n] * sin(2*Float.pi * Float(n) * Float(i) / Float(Times.count))
    }
...
//CSV
let DocumentDirURL: String! = "/Users/niquit/WIZUT/PTD/lab2/"
